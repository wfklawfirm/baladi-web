  ...
  if (message.role === 'user') {
    return (
      <div className="flex justify-start animate-slide-up">
        <div className="max-w-[75%] bg-burgundy text-white rounded-2xl rounded-tr-sm px-4 py-3">
            ...
            <div className="flex items-center gap-3">
              ...
              {message.duration_ms != null && (
                <span className="text-xs text-warm-muted">
                  {(message.duration_ms / 1000).toFixed(1)}ث
                </span>
              )}
              {message.chunks_used != null && (
                <span className="text-xs text-warm-muted">
                  {message.chunks_used} مصادر
                </span>
              )}
            </div>
        ...
        {sourcesOpen && message.sources && message.sources.length > 0 && (
          <div className="mt-2 space-y-2 animate-fade-in">
            {message.sources.map((s, i) => (
              <div
                key={s.chunk_id}
                className="bg-warm-bg border border-warm-border rounded-xl px-4 py-3"
              >
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[10px] bg-burgundy/10 text-burgundy px-1.5 py-0.5 rounded">
                      {s.domain_name}
                    </span>
